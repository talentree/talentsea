...
export class AdminConsolePage extends NavElement {
            ...
            <div class=" columns is-mobile is-centered is-full ">
                <div class=" column is-11 ">
                    ...
                    <div class="home-position">
                        ...
                        ><!--TODO: controllare perche grandezza icone non va-->
                    </div>
                    <hr />
                </div>
            </div>
    ...
    proceedWithP5Script(game) {
        ...
        if (!this.istanzaP5) {
            ...
            this.gameUploadReference = setInterval(() => {
                if (this.adminConsoleP5.gameIsPlaying) {
        ...
        if (!this.adminPeer) {
            ...
            this.adminPeer.on('connection', (conn) => {
                let teamId = this.convertPeerIdToTeam(conn.peer);
                conn.on('data', (data) => {
                    this.adminConsoleP5.gameData.teams[teamId].inputs = data;
                });
                conn.on('open', () => {
                    console.log(`started connection with ${conn.peer} team: ${teamId}`);
                    //quando il team si connette, lo aggiungo alle squadre connesse
                    this.activeConnections.push(conn);
                    //setto isUsed la squadra
                    if (this.adminConsoleP5.gameData.teams[teamId] != null) {
                        this.adminConsoleP5.gameData.teams[teamId].outputs.isUsed = true;
                    }
                ...
                conn.on('close', () => {
                    ...
                    if (this.adminConsoleP5.gameData.teams[teamId] != null) {
                        this.adminConsoleP5.gameData.teams[teamId].outputs.isUsed = false;
                    }
    ...
    disconnectedCallback() {
        ...
        if (this.listenToChangesFunction) {
            this.listenToChangesFunction();
        }
        //elimino l'istanza di p5
        if (this.istanzaP5) {
            this.istanzaP5.remove();
        }
        //elimino setInterval
        if (this.gameUploadReference) {
            clearInterval(this.gameUploadReference);
        }
        //elimino setInterval (comunicazione ai team)
        if (this.peerUpdatesReference) {
            clearInterval(this.peerUpdatesReference);
        }
    ...
    sendUpdatesToPeers(singlePeerName = '') {
        Object.keys(this.adminConsoleP5.gameData.teams).forEach((teamName) => {
            this.activeConnections.forEach((connection) => {
                if (this.convertPeerIdToTeam(connection.peer) == teamName && (singlePeerName == '' || singlePeerName == teamName)) {
                    let dataToSend = {
                        info: {},
                        teams: {},
                    };
    ...
    convertPeerIdToTeam(peerId) {
        return peerId.slice(peerId.indexOf('_') + 1);
    }
}
